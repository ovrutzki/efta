    ...
    <>
              ...
              <FormControl sx={{ m: 1, minWidth: 100 }}>
                ...
                <Select
                  ...
                  input={<OutlinedInput label="Month" />}>
                    {Object.entries(months).map((month)=>{return <option value={month[0]}>{`${month[0]} - ` + (Number(month[1]))}</option>})}
      ...
      '::-webkit-scrollbar': {
        ...
        // {[[1,"er"],[2,"er"],[3,"er"]].map((day)=>{
          let month_for_box = months[`${month}`] < 10
          ? `0${months[`${month}`]}`
          : `${months[`${month}`]}`;
                    return(
                <Box id={`${month_for_box}-${day[0]}-${year}`} onClick={()=>(setSelectedDay(`${month_for_box}-${day[0]}-${year}`))} sx={{display:"flex", textAlign:"center",flexDirection:"column",alignItems:"center", justifyContent:"space-between",paddingTop:"5px",mx:"3px", paddingBottom:"15px",width:"47px", height:"100px", backgroundColor: selectedDate === `${month_for_box}-${day[0]}-${year}` ?"#4E4E61" : "#989CA9"  , borderRadius:"16px"}}>
